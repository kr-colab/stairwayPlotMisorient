...
public class SFS_lnL02 extends Problem{
    ...
    public double getLogLikelihood(double[] Theta, double pMisPol){
        getInitialize(Theta);
        double totall=0;
        ...
        for(int i=1;i<n;i++){
            if(obs[i]){
                currP = Math.exp(logexi[i])*(1-pMisPol) + Math.exp(logexi[n-i])*pMisPol;
                totall+=Math.log(currP)*xi[i]-gammln(xi[i]+1.0);
            }
            else{
    ...
    public double fitness(double[] x) {
        ...
        return -getLogLikelihood(Theta, x[x.length-1]);
    }
    @Override
