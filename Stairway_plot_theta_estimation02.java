...
public class Stairway_plot_theta_estimation02 {
     public static void main(String[] args)throws Exception{
        ...
        for(int ii=0;ii<nsfs;ii++){
            ...
            double[] bestest=new double[dim+1];
            int nfeasible=0;
            for(int i=0;i<numRuns;i++){
                ...
                if(result.feasible){
                    if(result.fitness<bestfit){
                        ...
                        bestest=new double[dim+1];
                        System.arraycopy(result.parameters, 0, bestest, 0, dim+1);
                        if(verbose){
                            System.out.println(i+" "+result.iterations+" "+result.fitness+" "+result.feasible+" "+result.parameters.length);
                            for(int j=0;j<dim+1;j++)System.out.print(bestest[j]+" ");
                            System.out.println();
                        }
                    }
                    nfeasible++;
            ...
            if(fitnesstrail){
                ...
                for(int i=0;i<dim+1;i++) out.print("\t"+bestest[i]);
                out.println();
            }
            int theta_begin=3;
            while(keepgoing){
                        ...
                        if(result.feasible){
                            ...
                            if(result.fitness<bestfit){
                                ...
                                if(!theta0){//avoid any theta=0
                                    ...
                                    if(verbose){
                                        System.out.println(i+" "+result.iterations+" "+result.fitness+" "+result.feasible+" "+result.parameters.length);
                                        for(int jj=0;jj<dim+1;jj++)System.out.print(bestest[jj]+" ");
                                        System.out.println();
                                    }
                                }
                            }
            ...
            for(int i=0;i<dim-1;i++)solution[i]=bestest[i]*L;
            solution[dim-1]=bestest[dim-1];
            for(int i=2;i<=nx;i++){
                if(bestsplit[i]){
                    if(i==2)out.print(i);
                    else out.print("\t"+i);
